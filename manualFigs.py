...
def chart(what):
    ...
    if what == 'mEq1':
        _ = None
        # m  1  K  [16]  alpha 0.4, ucb1 ucb part 1, ucb1-s ucb part 1, TS fixed
 ...
 0.65247194 ,0.82279293 ,0.77109717, 0.9852433]}
        for i in range(2):
            plot_fixed_m(i, [16], np.arange(1, 11) * 100, naiveUCB1_, TS, ADAETC_, ETC_,
                         _, UCB1_stopping_, _, _, params_)
        # m  1  K  [16]  alpha 0.4, ucb1 ucb part 1, ucb1-s ucb part 1, TS fixed



        # m  1  K  [16]  alpha 0, for no UCB and TS figs
        params_ = {'numOpt': 1, 'alpha': 0, 'totalSim': 50, 'numArmDists': 100, 'c': 4, 'delta': 0, 'm': 1,
                   'Switch': 'no', 'NADA': 'no'}

        ADAETC_ = {'regret': [53.57680598 , 79.09377134 , 96.29813585, 109.67900559, 126.14830016,
 137.21403057 ,150.98537955 ,160.92225996 ,169.67125626 ,180.23990523],
                   'standardError': [0.41955339 ,0.66764518 ,0.92478658, 0.96503449, 1.05154646, 1.25247113,
 1.54103494 ,1.54398925, 1.56293263, 1.88880645]}

        ETC_ = {'regret': [60.32600598,  93.17257134 ,122.40013585, 137.60000559, 156.05570016,
 183.96363057, 201.86517955, 217.53745996, 229.04585626, 246.74970523],
                'standardError': [0.41628679, 0.60591247 ,0.87374815 ,1.00239982 ,0.91521838 ,1.03284089,
 1.23973102 ,1.30228638 ,1.22854362, 1.37736266]}

        UCB1_stopping_ = {'regret': [60.20140598 , 88.19977134, 113.47433585, 125.66080559 ,138.89030016,
 156.06983057 ,169.24417955 ,181.83525996, 192.48845626 ,202.33470523],
                          'standardError': [0.41699296, 0.621419  , 0.8278018 , 1.01448536 ,1.1102797  ,1.24683984,
 1.60568233 ,1.47060625, 1.63419372, 1.96839572]}
        plot_fixed_m(1, [16], np.arange(1, 11) * 100, _, _, ADAETC_, ETC_,
                     _, UCB1_stopping_, _, _, params_)
        # m  1  K  [16]  alpha 0, for no UCB and TS figs
 ...
 207.88965904 ,235.4898181 , 246.53133286, 304.34657064 ,306.11734375],
  ...
  9.01011964, 10.71469655, 12.70740855 ,13.79260468]}
    ...
    if what == 'rewardGen':
        ...
        # for m_cohort = 5, K = 20, T = 200 - UCB1-I-s with ucb part 1
        algs = {'ADA-ETC': {'reward': np.array([20.7918868, 21.61003976, 21.84315322, 21.97463225, 21.98947917]),
                ...
                'UCB1-I-s': {'reward': np.array([20.03820862, 20.14576446, 20.46811594, 20.56964811, 20.57458333]),
                             'stError': np.array([0.02717224, 0.01316413, 0.02487751, 0.01743683, 0.01221855])},
        ...
        # for m_cohort = 5, K = 10, T = 100 - UCB1-I-s with ucb part 1
        algs = {'ADA-ETC': {'reward': np.array([10.12263158, 10.6071308 , 10.8414862 , 10.95083906, 10.98857698]),
                ...
                'UCB1-I-s': {'reward': np.array([10.08027921, 10.46505707, 10.62027601, 10.69989407, 10.77957447]),
                             'stError': np.array([0.0163792 , 0.01682211, 0.01259598, 0.01368905, 0.01961391])},
