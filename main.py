...
def generateTwoFartherArms(T_list, numArmDists, pw=1 / 3):
    ncol = 2 * len(T_list)
    armInstances = np.zeros((numArmDists, ncol))
...
def naiveUCB1(armInstances, startSim, endSim, K_list, T_list):
    ...
    for t in range(numT):
        ...
        for a in range(numInstance):
            ...
            for j in range(endSim - startSim):
                ...
                subOptRewards_sim[a] += np.abs(cumulative_reward[0] - cumulative_reward[1]) / sum(cumulative_reward)
    ...
    print(stError)
    print("Ratio of difference between cumulative rewards to the total cumulative rewards")
    print(subOptRewards)
...
def ADAETC(varyingK, armInstances, startSim, endSim, K_list, T_list):
    if varyingK:  # fix T and vary K values
        ...
        regret = np.zeros(numK)
        stError = np.zeros(numK)
        subOptRewards = np.zeros(numK)
        subOptRewardsTot = np.zeros(numK)
        for t in range(numK):
            K = K_list[t]
            regret_sim = np.zeros(numInstance)
            subOptRewards_sim = np.zeros(numInstance)
...
if __name__ == '__main__':
    ...
    armInstances = generateTwoFartherArms(T_list, numArmDists, pw)
